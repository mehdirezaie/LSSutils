...
class DATA(object):
    def __init__(self, INPUT, ax=list(range(17)), OUTPUT='./dr5linquadfit', split=False):
        self.ax = ax
        if split:
        ...
        popt, pcov = curve_fit(quad, self.XX, self.Y, p0=[0 for i in range(2*len(ax)+1)],
                                 sigma=self.Ye, absolute_sigma=True)
        self.quad  = (popt, pcov)
    ...
    def plotbfitparam(self, path4png='bfitdr5.pdf'):
        ...
        self.LABELS = ['EBV', 'NStar']+[l+s for l in ['depth','seeing','airmass','skymag','exptime'] for s in 'rgz']
    ...
    def savemodels(self, path4models):
        ...
        hp.write_map(path4models+'lin-weights.hp256.fits', mapi, fits_IDL=False, dtype=np.float64)
        mapj = np.zeros(12*256**2)
        mapj[self.hpix] = quad_model
        hp.write_map(path4models+'quad-weights.hp256.fits', mapj, fits_IDL=False, dtype=np.float64)
#
from argparse import ArgumentParser
ap = ArgumentParser(description='Multivariate linear/quadratic regression')
ap.add_argument('--input',  default='/Volumes/TimeMachine/data/dr5.0/eboss/ebossELGngal-features.fits')
ap.add_argument('--ax', nargs='*', type=int, default=[i for i in range(17)])
ap.add_argument('--output', default='/Volumes/TimeMachine/data/dr5.0/eboss/')
