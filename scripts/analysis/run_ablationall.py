...
import numpy as np
import sys
sys.path.append('../DESILSS')
import NN
from time import time


#
CONFIG = dict(nchain=3, batchsize=1024, nepoch=200, Units=[0], tol=0.0, learning_rate=0.001, scale=0.0)
# 
def fix(data, ax=list(range(17))):
    dt = np.dtype([('label', '>f8'), ('hpix', '>i8'), ('features', ('>f8', len(ax))), ('fracgood', '>f8')])
...
if rank == 0:
    ...
    ap.add_argument('--index', nargs='*', type=int, default=[0,17])
