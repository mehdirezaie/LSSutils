...
path       = '/home/mehdi/data/desi/mockchallenge/stage3/'
mode       = '2d'      # r, mu
dr         = 1.0       # Mpc/h
nmu        = 120
rmin       = 1.e-15
...
ouname2    = path + f'{ESTIMATOR2}_{BINNING}_{LASTNAME}_{MOCKNAME}_{version}_{space}_{zmin}.txt'
input_name2= path + 'UNIT_lightcone_multibox_ELG_footprint_nz_1xdata_1.ran_NGC.dat'
#input_name2= path + 'UNIT_lightcone_multibox_ELG_footprint_nz_5xdata_NGC.dat'#'UNIT_lightcone_ELG_rand_NGC_Z_rsd.dat'
input_name1= path + 'UNIT_lightcone_multibox_ELG_footprint_nz_NGC.dat'#'UNIT_lightcone_ELG_NGC_Z_rsd.dat'

    
# read data
data1    = nb.CSVCatalog(input_name1, ['RA','DEC', 'Z', 'Z_RSD', 'NZ'])
randoms1 = nb.CSVCatalog(input_name2, ['RA','DEC', 'Z', 'Z_RSD', 'NZ'])
...
# specify cosmology
cosmo = Cosmology(h=0.6774).match(Omega0_m=0.3089)


#--- perform selection
valid = (data1[redname] >= zmin) & (data1[redname] <= zmax)
data1 = data1[valid]

valid = (randoms1[redname] >= zmin) & (randoms1[redname] <= zmax)
randoms1 = randoms1[valid]

Pzero  = 5000
data1['Weight']    = 1.0 / (1 + data1['NZ'] * Pzero)
randoms1['Weight'] = 1.0 / (1 + randoms1['NZ'] * Pzero)
