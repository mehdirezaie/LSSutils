...
def get_cl(ngal, nran, mask, select_fun=None, systematics=None, njack=20, lmax=None, comm=None):    
    # initialize AnaFast
    ...
    if systematics is not None:        
        ...
        for i in range(start, end):
            print('.', end='')
            systematic_i = overdensity(systematics[:, i], weight, mask, is_sys=True)            
            cl_ss_list.append(af(systematic_i, weight, mask, **af_kw))
            cl_sg_list.append(af(delta, weight, mask, map2=systematic_i, weight2=weight, mask2=mask, **af_kw))
