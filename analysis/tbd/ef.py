...
def ipix2neighbors_array(ipix, nside=4096, layer=1):
    ...
    for i,ip in enumerate(ipix):
        ...
        if i % 100000 == 0:
            t2 = time()
            print(i,' from ', totnpix, 'in ', t2-t1, 'ms')# print(nghbrs[cpix], ip)
            t1 = t2
        if len(nghbrs) == nnghbrs:
            ipix_list.append(True)
            nghbrs_list.append(nghbrs)
            npix += 1
        else:
            ipix_list.append(False)
    nghbrs_array = np.array(nghbrs_list).reshape(npix, nnghbrs)
    ipix_array = np.array(ipix_list)
    return nghbrs_array, ipix_array
...
def extract_features(input_data_sample, input_data_all, nside=4096, layer=1):
    ...
    #
    #
    nghbrs_array, input_d_ipix_mask =  ipix2neighbors_array(input_data_ipix_sample, nside=nside, layer=layer)
    comp_neighbors = neighbors_mask(nghbrs_array, input_data_ipix_all)
    comp_nghbrs_array = nghbrs_array[comp_neighbors]
    #
    #
    input_data_sample = input_data_sample[input_d_ipix_mask]
...
def main(sys):
    ...
    y['hpix'] = hpix
    y['features'] = features
    np.savez_compressed('layersallv2-'+str(layers), y)
