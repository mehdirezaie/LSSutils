...
slices="main linstar2" #"low mid" #"main highz" # options are "main highz low mid z1 z2 z3"
maps="known" # options are "all known" but known is enough
samples="mainlinstar2" # options are lowmidhighz mainhighz / only 1: mainlinmse mainlinp mainmse mainhighz mainwocos mainstar mainstarg mainlinstar
    ...
    do
        ...
        do
            ...
            do
                ...
                output_path=${input_dir}lin_star_${map}/hp/
                echo ${output_path} ${axes}
                python $nnfit -i ${input_path} -o ${output_path} --model lin --loss mse -ax ${axes} -bs ${batchsize} -fl
            done
        done
    done
fi
    ...
    do
        ...
        do
            ...
            do
                ...
                axes=$(get_axes ${map})
                output_path=${input_dir}lin_star2_${map}
                echo ${output_path} ${lr} ${axes} ${slice}
                python $nnfit -i ${input_path} -o ${output_path} --model lin --loss mse -ax ${axes} -lr ${lr} -bs $batchsize --nn_structure ${nn_structure[@]} -ne $nepoch -nc $nchains -k
            done
        done
    done
fi
...
then
    ...
    do
        ...
        do
            ...
            python $swap -m ${map} -n ${nside} -s ${slices} -c ${cap} -v ${release} --method lin_star2 # 1+1 z
        done
    done
fi
