...
# --- inputs
path_cl  = sys.argv[1:3]
path_cov = sys.argv[3:5]
region   = sys.argv[5:7]  # for window
output   = sys.argv[7]
...
ndim     = 5      # Number of parameters/dimensions
...
def logpost(params):
    fnl, b1, sn1, b2, sn2 = params
    return lg.logpost([fnl, b1, sn1]) + lg2.logpost([fnl, b2, sn2])
...
# scipy optimization        
np.random.seed(SEED)
res = minimize(neglogpost, [1.0, 1.0, 1.0e-7, 1.0, 1.0e-7], method='Powell')
