'''
    ...
    Examples to call it directly
        python xi.py --galmap /Volumes/TimeMachine/data/eboss/eBOSS_DR7/eBOSSDR7.256.fits
                     --ranmap /Volumes/TimeMachine/data/eboss/eBOSS_DR7/random.256.fits  
                     --mask /Volumes/TimeMachine/data/eboss/eBOSS_DR7/hpmask.256.fits 
                     --njack 0 --nside 256 
                     --oudir /Volumes/TimeMachine/data/eboss/eBOSS_DR7/clustering/ 
                     --ouname ebossdr7xi_2pc_wnn 
                     --selection /Volumes/TimeMachine/data/eboss/eBOSS_DR7/regression/nn-weights.hp256.fits
'''
...
def split_jackknife(theta, phi, weight, delta, sysm, njack=20):
    '''
        Splits the data into njack regions
    ...
    for i in range(theta.size):
        ...
        if frac >= f:
            ...
            phi_l   = []
            delta_l = []
        elif (i == theta.size-1) and (frac > 0.9*f): # include remainder if > 90% f
            theta_L.append(theta_l)
            phi_L.append(phi_l)
            frac_L.append(frac)
            delta_L.append(delta_l)
...
def hpupgrade(mapin, res_o, res_i):
    ...
    inputs:
        ...
        upgrade/downgrade a HP map resolution
        currently calls hp.ud_grade
    '''
...
class XI_JACK(object):
    ...
    def run(self, thetamax=10.):
        bw    = hp.nside2resol(self.nside)
        bins  = np.arange(bw, np.deg2rad(thetamax), bw)[::-1]
